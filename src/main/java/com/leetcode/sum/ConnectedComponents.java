		...
		if(null != edges && 0 != edges.length) {
			for(int[] edge : edges) {
				...
				if( compNum1 == -1 && compNum2 == -1) {
					...
					components.get(compNum1).add(edge[1]);
					compNum2 = compNum1;
				} else if ( compNum1 == -1 && compNum2 != -1) {
					components.get(compNum2).add(edge[0]);
					compNum1 = compNum2;
				} else if( compNum1 != -1 && compNum2 != -1 && compNum1 != compNum2) {
					components.get(compNum1).addAll(components.get(compNum2));
					components.remove(compNum2);
				}
			}
		}
		return components.size() + (n - components.stream().mapToInt(comp -> comp.size()).sum());
	}
